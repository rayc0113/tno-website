import type { Product } from "@/types/product";

const anchorFittings: Product = {
  slug: "anchor-fittings",
  name: "導纜孔、繫纜樁、錨座",
  nameEn: "Chocks, Bollards & Anchor Pedestals",
  category: "造船用甲板上艤品、錨座配件",
  shortDescription: "高強度船用錨座配件，確保錨泊系統安全可靠",
  shortDescriptionEn:
    "High-strength marine anchor fittings engineered for safe, dependable mooring systems.",
  description:
    "欣展提供完整的船用錨座配件系列，包括錨鏈艙口、錨機座板、止鏈器、錨鏈導向器及相關緊固配件。採用高強度船用鋼材製造，依照 IACS 及各船級社規範設計，確保錨泊作業的安全性與可靠性，適用於各型商業船舶。",
  descriptionEn:
    "TNO offers a full catalog of marine anchor fittings — chain pipes, windlass mounting plates, chain stoppers, chain guides, and matching fasteners. Forged and cast from high-strength marine steel and designed to IACS and classification-society specifications, these fittings ensure safe, reliable mooring operations across every class of commercial vessel.",
  images: [
    "/images/products/_placeholder.webp",
  ],
  specs: [
    { label: "材質", value: "高強度船用鋼 / 鑄鐵" },
    { label: "表面處理", value: "熱浸鍍鋅 / 防腐底漆" },
    { label: "設計規範", value: "依各船級社規格" },
    { label: "試驗壓力", value: "依 IACS 規定荷載測試" },
    { label: "認證", value: "ABS、ClassNK、DNV、LR" },
  ],
  specsEn: [
    { label: "Material", value: "High-strength marine steel / Cast iron" },
    { label: "Surface Finish", value: "Hot-dip galvanizing / Anti-corrosion primer" },
    { label: "Design Standards", value: "Per classification society specifications" },
    { label: "Proof Load", value: "Tested per IACS load requirements" },
    { label: "Certifications", value: "ABS, ClassNK, DNV, LR" },
  ],
  features: [
    "高強度鑄造或鍛造製程，承載能力強",
    "符合 IACS 錨泊設備設計規範",
    "完善的防腐處理，延長使用壽命",
    "通過多國主要船級社認證",
    "可依船型噸位客製化規格",
  ],
  featuresEn: [
    "High-strength cast or forged construction with excellent load capacity",
    "Compliant with IACS mooring-equipment design standards",
    "Thorough anti-corrosion treatment for a long service life",
    "Certified by major classification societies worldwide",
    "Custom specifications to match vessel tonnage",
  ],
  applications: ["錨鏈艙口蓋", "錨機座板系統", "止鏈及導鏈設備", "船舶錨泊系統配套"],
  applicationsEn: [
    "Chain pipe covers",
    "Windlass mounting plate systems",
    "Chain stoppers and chain guides",
    "Marine mooring system packages",
  ],
  publishedAt: "2024-03-01",
  // 2026-07-24 會議決議下架（保留資料，不刪除）
  isPublished: false,
};

export default anchorFittings;
