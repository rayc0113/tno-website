import type { Product } from "@/types/product";

const rudderShaftSystem: Product = {
  slug: "rudder-shaft-system",
  name: "舵軸、舵鑄件、舵葉、軸架、舵架鑄件",
  nameEn: "Rudder Stocks, Rudder Castings, Rudder Blades & Shaft Brackets",
  category: "造船用舵軸系統",
  shortDescription: "高精度船用舵軸系統，確保船舶操控穩定性與安全性",
  shortDescriptionEn:
    "Precision-machined marine rudder & shaft systems delivering stable, reliable steering performance.",
  description:
    "欣展提供完整的船用舵軸系統，包括舵軸、舵托、舵機支架及相關緊固配件。採用高強度合金鋼製造，通過 ABS、ClassNK 等主要船級社認證，確保在各種海況下的操舵穩定性與可靠性，適用於各類型商業船舶與特種船。",
  descriptionEn:
    "TNO delivers complete marine rudder and shaft systems — including rudder stocks, rudder carriers, steering-gear mounts, and all related fasteners. Machined from high-strength alloy steel and certified by ABS, ClassNK, and other major classification societies, these systems ensure stable, dependable steering performance across all sea states — suitable for every class of commercial vessel and specialty ship.",
  images: [
    "/images/products/_placeholder.webp",
  ],
  specs: [
    { label: "材質", value: "高強度合金鋼 / 不鏽鋼" },
    { label: "表面處理", value: "鍍鉻 / 防腐塗層" },
    { label: "適用船型", value: "商業貨輪、散裝船、特種船" },
    { label: "加工精度", value: "依 ISO 標準公差" },
    { label: "認證", value: "ABS、ClassNK、DNV、BV" },
  ],
  specsEn: [
    { label: "Material", value: "High-strength alloy steel / Stainless steel" },
    { label: "Surface Finish", value: "Chrome plating / Anti-corrosion coating" },
    { label: "Vessel Types", value: "Commercial cargo, bulk carriers, specialty vessels" },
    { label: "Machining Tolerance", value: "Per ISO standard tolerances" },
    { label: "Certifications", value: "ABS, ClassNK, DNV, BV" },
  ],
  features: [
    "高強度合金鋼，承載能力優異",
    "精密加工，確保操舵靈活穩定",
    "通過多項國際船級社認證",
    "耐腐蝕塗層，適應惡劣海洋環境",
    "可依船型規格客製化設計",
  ],
  featuresEn: [
    "High-strength alloy steel with excellent load-bearing capacity",
    "Precision machining for responsive, stable steering",
    "Certified by multiple international classification societies",
    "Corrosion-resistant coatings that withstand harsh marine environments",
    "Custom designs to match each vessel's specifications",
  ],
  applications: ["商業貨輪舵系統", "散裝船操舵設備", "特種船舵機配套", "船舶改裝維修"],
  applicationsEn: [
    "Commercial cargo vessel rudder systems",
    "Bulk-carrier steering equipment",
    "Specialty-ship steering gear packages",
    "Vessel refit and repair",
  ],
  publishedAt: "2024-03-01",
  // 2026-07-24 會議決議下架（保留資料，不刪除）
  isPublished: false,
};

export default rudderShaftSystem;
